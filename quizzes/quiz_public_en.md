# MIHENK Quiz

Answer the questions below. For **multiple-choice** items write ONLY the correct option letter (A–E); for **short-answer** items write ONLY the answer (at most 7 words). Do not explain. Give each reply with its question number (e.g. `1. C`).


---

**1.** [Bilimsel Makale Anlama · L3] A study found that ice cream sales and drowning incidents both rise in summer. What is the most correct conclusion from this data?
- A) Ice cream causes drowning
- B) The two may share a common cause (e.g. hot weather)
- C) Drownings increase ice cream sales
- D) There is no relationship at all

**2.** [Bilimsel Makale Anlama · L4] A method reduced an error rate from 50% to 10%. What is the absolute reduction in percentage points? (number)
_(short answer)_

**3.** [Biyoloji · L3] A plant's leaves begin to wilt and the soil is dry. At the cellular level, the most likely cause is:
- A) A drop in turgor pressure in the cells
- B) An increase in the number of chloroplasts
- C) Faster mitosis
- D) Shrinking of the nucleus

**4.** [Biyoloji · L4] Which sex chromosome is present in human males but not in females? (letter)
_(short answer)_

**5.** [Coğrafya · L3] A person walks 6 km north and then 8 km east. What is the straight-line distance back to the starting point?
- A) 14
- B) 10
- C) 2
- D) 48

**6.** [Coğrafya · L4] A 300 km river drops from 1200 m at its source to 0 m at its mouth. What is its average gradient in m/km?
_(short answer)_

**7.** [Debugging · L3] The programmer expected the first function to return 0, but it returns 2:
funcs = [lambda: i for i in range(3)]
print(funcs[0]())
Why?
- A) lambdas are unsupported
- B) i is captured by closure and read at call time, when the loop has ended with i = 2
- C) range is wrong
- D) funcs[0] is actually the second element

**8.** [Debugging · L4] A recursive factorial that raises RecursionError is missing what? (two words)
_(short answer)_

**9.** [Ekonomi · L3] A product costs 80 and is sold at a 25% profit. What is the selling price?
- A) 100
- B) 105
- C) 60
- D) 20

**10.** [Ekonomi · L4] A good whose quantity demanded falls 20% when its price rises 10% has demand that is elastic or inelastic? (one word)
_(short answer)_

**11.** [Felsefe · L3] 'All cats are animals. Some animals are dogs. Therefore some cats are dogs.' Which is true of this argument?
- A) Valid and sound
- B) Valid but unsound
- C) Invalid
- D) Its premises are false

**12.** [Felsefe · L4] Can a single counterexample refute an inductive generalization such as 'all swans are white'? (yes/no)
_(short answer)_

**13.** [Fizik · L3] A car accelerates from rest at 3 m/s² for 4 seconds. How far does it travel?
- A) 12
- B) 24
- C) 48
- D) 6

**14.** [Fizik · L4] An object with kinetic energy 36 J has a mass of 2 kg. What is its speed in m/s? (KE = ½mv²)
_(short answer)_

**15.** [Grafik Analizi · L3] In a distance-time line graph, the reading is 0 m at 0 s, 20 m at 2 s, and 40 m at 4 s. What is the slope (speed) in m/s?
- A) 10
- B) 20
- C) 5
- D) 40

**16.** [Grafik Analizi · L4] In a pie chart, the Food slice is 30%. If the total budget is 8000, how much goes to food?
_(short answer)_

**17.** [Günlük Muhakeme · L3] A box holds 5 red and 3 blue balls. Drawing blindly, what is the fewest balls you must take to be sure of getting 2 of the same color?
- A) 2
- B) 3
- C) 4
- D) 6

**18.** [Günlük Muhakeme · L4] Two taps can fill a pool: one in 3 hours and the other in 6 hours. Working together, how many hours does it take to fill the pool?
_(short answer)_

**19.** [Hukuk · L3] Rule: 'Employees are off on weekends. However, those on the duty roster work on Saturday.' Mary is on the duty roster. On Saturday, Mary:
- A) is off
- B) works
- C) may choose to come
- D) is dismissed

**20.** [Hukuk · L4] Under the principle 'no crime without law', can an act not defined as a crime be punished? (yes/no)
_(short answer)_

**21.** [Kimya · L3] In 2H2 + O2 → 2H2O, if 4 mol of H2 react completely, how many moles of water are produced?
- A) 2
- B) 4
- C) 8
- D) 1

**22.** [Kimya · L4] How many moles are in 58.5 g of NaCl? (molar mass of NaCl is 58.5 g/mol)
_(short answer)_

**23.** [Mantık · L3] Four runners A, B, C, D finish a race in distinct positions. A finishes before B, C finishes before A, and D is not last. Who finishes last?
- A) A
- B) B
- C) C
- D) D

**24.** [Mantık · L4] A says 'B is a knight', B says 'C is a knave', and C says 'A and B are of different kinds'. Knights tell the truth and knaves lie. How many of them are knights?
_(short answer)_

**25.** [Matematik · L3] A store buys an item for 50 dollars, marks it up by 40%, then sells it at 15% off the marked price. What is the store's profit as a percentage of the cost?
- A) 25
- B) 19
- C) 21
- D) 11

**26.** [Matematik · L4] How many integers from 1 to 100 inclusive are divisible by 4 or by 6?
_(short answer)_

**27.** [Programlama · L3] What does the second print output?
def add(x, lst=[]):
    lst.append(x)
    return lst
print(add(1))
print(add(2))
- A) [2]
- B) [1, 2]
- C) [1]
- D) [2, 1]

**28.** [Programlama · L4] How many 1-bits are in the binary representation of the number 27?
_(short answer)_

**29.** [Sayısal Akıl Yürütme · L3] A price first rises by 20% and then falls by 20% off the new price. What is the net change from the start?
- A) +4%
- B) -4%
- C) No change
- D) -20%

**30.** [Sayısal Akıl Yürütme · L4] The average age of 4 people is 30. If a 20-year-old joins them, what is the new average age of the 5 people?
_(short answer)_

**31.** [Tablo ve Veri Yorumu · L3] Products' cost and sale prices (Product: Cost/Sale) — A: 40/60, B: 30/36, C: 50/80. Which product has the highest profit rate (profit ÷ cost)?
- A) A
- B) B
- C) C
- D) All equal

**32.** [Tablo ve Veri Yorumu · L4] Class table (Class: Students/Passed) — 7A: 40/30, 7B: 25/20. Which class has the higher pass rate? (7A/7B)
_(short answer)_

**33.** [Tarih · L3] The spread of paper and printing accelerated the flow of information. This most directly facilitated which development?
- A) The rapid spread of Reformation ideas
- B) The strengthening of feudalism
- C) The invention of the wheel
- D) The first emergence of agriculture

**34.** [Tarih · L4] In which year did the Western Roman Empire fall? (year)
_(short answer)_

**35.** [Türkçe Dil Bilgisi · L3] The Turkish suffix '-ince' in 'görünce' (upon seeing) forms which kind of verbal?
- A) A participle (adjectival)
- B) A verbal noun
- C) A converb (adverbial verb form)
- D) A finite verb

**36.** [Türkçe Dil Bilgisi · L4] In the Turkish 'ne ... ne ...' construction, must the verb be affirmative or negative in form? (one word)
_(short answer)_

**37.** [Çok Adımlı Muhakeme · L3] A store had 100 kg of flour. On day one, 20% of the total was used; on day two, 25% of the remainder was used. How many kg remain?
- A) 60
- B) 55
- C) 40
- D) 65

**38.** [Çok Adımlı Muhakeme · L4] A pool holds 600 liters and currently has 300 liters. A tap adds 20 liters per minute while a drain removes 5 liters per minute. In how many minutes will the pool be completely full?
_(short answer)_

**39.** [İngilizce Dil Bilgisi · L3] Choose the correct form: 'If I ___ rich, I would travel the world.'
- A) am
- B) was
- C) were
- D) will be

**40.** [İngilizce Dil Bilgisi · L4] Name the part of speech of the word 'quickly'. (one word)
_(short answer)_



---

## ANSWER KEY

```
1. B
2. 40  (kabul: 40 points)
3. A
4. Y  (kabul: Y chromosome, y)
5. B
6. 4  (kabul: 4 m/km)
7. B
8. base case  (kabul: a base case, base condition)
9. A
10. elastic
11. C
12. yes  (kabul: evet)
13. B
14. 6  (kabul: 6 m/s)
15. A
16. 2400  (kabul: 2,400)
17. B
18. 2  (kabul: 2 hours)
19. B
20. no  (kabul: hayır)
21. B
22. 1  (kabul: 1 mol)
23. B
24. 2  (kabul: two)
25. B
26. 33
27. B
28. 4
29. B
30. 28
31. C
32. 7B  (kabul: 7-B, 7 B, 7b)
33. A
34. 476  (kabul: 476 CE, 476 AD)
35. C
36. affirmative
37. A
38. 20  (kabul: 20 minutes)
39. C
40. adverb  (kabul: an adverb)
```
